...
public class SeekBehaviour : SteeringBehaviour
{
    ...
    [SerializeField]
    private float targetRechedThreshold = 0.5f, radius = 2f;
    ...
    private void OnDrawGizmos()
    {
        ...
        if (Application.isPlaying && interestsTemp != null)
        {
            if (interestsTemp != null)
            {
                ...
                {
                    Gizmos.DrawRay(transform.position, Directions.eightDirections[i] * interestsTemp[i]*radius);
                }
                if (reachedLastTarget == false)
                {
                    Gizmos.color = Color.red;
                    Gizmos.DrawSphere(targetPositionCached, 0.06f);
                }
            }
        }
    }
